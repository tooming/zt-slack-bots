...
For seeing status: "/standup status"';
			...
			$result = $this->getStandupStatus($team_id, $channel_name);
			if(count($result) > 0) {
...
';
			...
			$this->deleteUserNotes($team_id, $channel_name, $user_id);
			$this->insertUserNotes($team_id, $channel_name, $user_id, $user_name, $text);
	...
	public function insertUserNotesSql() {
		return "INSERT INTO standups (team_id, channel_name, user_id, user_name, `text`, `date`, added_time)
				VALUES (?, ?, ?, ?, ?, CURDATE(), NOW())";
	}

	public function getStandupStatus($team_id, $channel_name) {
		return $this->fetchSqlResult($this->getStandupStatusSql(), [$team_id, $channel_name]);
	}

	public function deleteUserNotes($team_id, $channel_name, $user_id) {
		$this->executeSql($this->deleteUserNotesSql(), [$team_id, $channel_name, $user_id]);
	}

	public function insertUserNotes($team_id, $channel_name, $user_id, $user_name, $text) {
		$this->executeSql($this->insertUserNotesSql(), [$team_id, $channel_name, $user_id, $user_name, $text]);
	}
}
